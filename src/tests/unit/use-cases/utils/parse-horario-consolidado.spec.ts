...
describe('calcularAulasPorSemana', () => {
  ...
  it('deve calcular corretamente para múltiplos dias', () => {
    expect(calcularAulasPorSemana('23M12')).toBe(4) // 2 aulas na segunda + 2 aulas na terça
    expect(calcularAulasPorSemana('234M1')).toBe(3) // 1 aula em cada dia (seg, ter, qua)
  })

  it('deve calcular corretamente para horários separados por vírgula', () => {
    expect(calcularAulasPorSemana('2M1,3M6,4M56')).toBe(4) // 1 + 1 + 2 aulas
  })
...
describe('calcularUltimoDiaAula', () => {
  it('deve calcular data final corretamente para horário simples', () => {
    const dataInicio = new Date('2025-07-01') // 1º de julho de 2025 (terça-feira)
    const resultado = calcularUltimoDiaAula('2M12', dataInicio, 4) // 4 aulas, 2 por semana
    
    // Deve terminar em 8 de julho de 2025
    expect(resultado).toEqual(new Date('2025-07-08'))
  })

  it('deve calcular data final para múltiplos dias por semana', () => {
    const dataInicio = new Date('2025-07-01') // 1º de julho de 2025
    const resultado = calcularUltimoDiaAula('23M12', dataInicio, 72) // 72 aulas, 4 por semana
    
    // 72 ÷ 4 = 18 semanas a partir de 1º de julho
    // Deve terminar em 29 de outubro de 2025
  ...
  it('deve calcular corretamente para horários em dias diferentes', () => {
    ...
    const resultado = calcularUltimoDiaAula('2M1,4M1', dataInicio, 8) // 8 aulas, 2 por semana (seg e qua)
    
    // 8 ÷ 2 = 4 semanas
    expect(resultado).toEqual(new Date('2025-07-24')) // 4ª quarta-feira
  })
  ...
  it('deve lidar com casos onde a data de início não coincide com dia de aula', () => {
    const dataInicio = new Date('2025-07-01') // terça-feira
    const resultado = calcularUltimoDiaAula('4M1', dataInicio, 4) // aulas só na quarta
    
    // Primeira aula será na quarta (02/07), última na 4ª quarta (23/07)
  ...
  it('deve calcular corretamente para cenário real da disciplina Banco de dados', () => {
    ...
    // Cenário: 4 aulas por semana (2 na segunda + 2 na terça)
    // 72 aulas ÷ 4 = 18 semanas
    // Início: 1º julho 2025 → Fim: 29 outubro 2025
    expect(resultado).toEqual(new Date('2025-10-29'))
  })
})
...
describe('Cenários de consolidação de horários', () => {
  it('deve consolidar corretamente: 2m1 + 2m2 + 3m1 + 3m2 = 23m12', () => {
    // Simulando o que seria gerado pela consolidação
    ...
    // Verificar que segunda e terça têm 2 aulas cada
  ...
  it('deve consolidar corretamente: 2m1 + 3m6 + 4m6 + 4m5 = 2m1,3m6,4m56', () => {
    ...
    expect(resultado).toHaveLength(3)
    expect(calcularAulasPorSemana(horarioConsolidado)).toBe(4) // 1 + 1 + 2
  ...
  it('deve lidar com horários em períodos diferentes no mesmo dia', () => {
    ...
    expect(resultado).toHaveLength(3)
    expect(calcularAulasPorSemana(horarioConsolidado)).toBe(6) // 2 + 2 + 2
    
    // Todos são na segunda-feira, mas em períodos diferentes
