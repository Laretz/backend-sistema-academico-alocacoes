...
describe("BuscarUsuariosUseCase", () => {
  it("deve listar usuários com filtro de busca", async () => {
    ...
    await repo.create({ nome: "Ana Maria", email: "ana@ex.com", senha: "h", role: "PROFESSOR", especializacao: "Matemática" });
    await repo.create({ nome: "Bruno", email: "bruno@ex.com", senha: "h", role: "ADMIN", especializacao: "Física" });
    await repo.create({ nome: "Carlos", email: "carlos@ex.com", senha: "h", role: "PROFESSOR", especializacao: "Geografia" });
  ...
  it("deve listar usuários sem filtro (página 1)", async () => {
    const repo = new InMemoryUsersRepository();
    await repo.create({ nome: "A", email: "a@ex.com", senha: "h", role: "PROFESSOR" });
