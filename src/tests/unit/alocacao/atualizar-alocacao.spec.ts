...
describe('Atualizar Alocação Use Case', () => {
    beforeEach(() => {
        ...
        sut = new AtualizarAlocacaoUseCase(alocacoesRepository, disciplinasRepository, turmasRepository, cursoDisciplinaRepository);
    });

    it('deve ser possível atualizar uma alocação', async () => {
        const cursoId1 = 'curso-1';
        const cursoId2 = 'curso-2';
        await turmasRepository.create({ id: 'turma-1', nome: 'Turma 1', num_alunos: 30, periodo: 1, turno: 'MATUTINO', semestre: 1, ativa: true, curso: { connect: { id: cursoId1 } } } as any);
        await turmasRepository.create({ id: 'turma-2', nome: 'Turma 2', num_alunos: 30, periodo: 1, turno: 'MATUTINO', semestre: 1, ativa: true, curso: { connect: { id: cursoId2 } } } as any);

        await disciplinasRepository.create({ id: 'disciplina-1', nome: 'Matemática', carga_horaria: 80, curso: { connect: { id: cursoId1 } } });
        await disciplinasRepository.create({ id: 'disciplina-2', nome: 'Física', carga_horaria: 60, curso: { connect: { id: cursoId2 } } });

        const cd1 = await cursoDisciplinaRepository.create({ id_curso: cursoId1, id_disciplina: 'disciplina-1' });
        const cd2 = await cursoDisciplinaRepository.create({ id_curso: cursoId2, id_disciplina: 'disciplina-2' });
    ...
    it('deve ser possível atualizar apenas o usuário da alocação', async () => {
        ...
        await turmasRepository.create({ id: 'turma-1', nome: 'Turma 1', num_alunos: 30, periodo: 1, turno: 'MATUTINO', semestre: 1, ativa: true, curso: { connect: { id: cursoId } } } as any);
        await disciplinasRepository.create({ id: 'disciplina-1', nome: 'Matemática', carga_horaria: 80, curso: { connect: { id: cursoId } } });
        const cd1 = await cursoDisciplinaRepository.create({ id_curso: cursoId, id_disciplina: 'disciplina-1' });
