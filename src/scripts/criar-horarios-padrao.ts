      ...
      for (const [turno, horarios] of Object.entries(horariosDefinidos)) {
        for (let i = 0; i < horarios.length; i++) {
          ...
          const horario_inicio = new Date(
            `1970-01-01T${horario.inicio.hora.toString().padStart(2, "0")}:${horario.inicio.minuto.toString().padStart(2, "0")}:00.000Z`
          );
          const horario_fim = new Date(
            `1970-01-01T${horario.fim.hora.toString().padStart(2, "0")}:${horario.fim.minuto.toString().padStart(2, "0")}:00.000Z`
          );
          ...
          if (!existente) {
            ...
            horariosCreated++;
            console.log(
              `✅ Criado: ${dia} ${turno}${numeroHorario} (${horario.inicio.hora}:${horario.inicio.minuto.toString().padStart(2, "0")} - ${horario.fim.hora}:${horario.fim.minuto.toString().padStart(2, "0")})`
            );
          }
        }
      }
